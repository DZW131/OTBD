...
from nnunetv2.inference.cotta_wrapper import CoTTAWrapper
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        self.cotta = CoTTAWrapper(network)
        self.cotta.student.train()
        self.tta_optimizer = torch.optim.SGD(self.cotta.student.parameters(), lr=1e-4, momentum=0.9)
    ...
    def get_val_from_key(self, json_file: str, my_key: str):
        key = my_key + ".nii.gz"
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                if ofile is not None:
                    ...
                    if "C" in val:
                        self.cotta.set_activated_mod(True)
                    else:
                        self.cotta.set_activated_mod(False)
                else:
                    print(f'\nPredicting image of shape {data.shape}:')

                # print(f'perform_everything_on_device: {self.perform_everything_on_device}')
                message_infer = f'local perform_everything_on_device: {self.perform_everything_on_device}'
                print(f'using cotta: {self.cotta.activated}')
                print(message_infer)
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.cotta.activated:
            self.tta_optimizer.zero_grad()
            with torch.enable_grad():
                loss = self.cotta.adapt_batch(x)
                self.tta_optimizer.step()
            self.cotta.student.eval()

        with torch.no_grad():
            if self.cotta.activated:
                prediction = self.cotta.student(x)
            else:
